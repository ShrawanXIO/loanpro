      ...
      <div className="bg-primary-600 text-white">
        <div className="max-w-4xl mx-auto px-6 py-16 text-center">
          ...
          <p className="text-xl text-blue-100 mb-8 max-w-xl mx-auto">
            Simple loan management for individual lenders. Track clients, loans, and payments — branded as your own.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/login" className="bg-white text-primary-600 font-semibold px-8 py-3 rounded-lg hover:bg-blue-50 transition-colors">
              Get Started Free
            </Link>
            <Link href="/login" className="border border-white text-white font-semibold px-8 py-3 rounded-lg hover:bg-white/10 transition-colors">
              Sign In
            </Link>
          </div>
        </div>
      ...
      <div className="bg-gray-50 py-16">
        <div className="max-w-3xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-white rounded-xl p-8 elev-1">
              <div className="text-sm font-semibold text-primary-600 uppercase tracking-wide mb-2">Free Forever</div>
              <div className="text-3xl font-bold mb-1">₹0</div>
              <div className="text-gray-400 text-sm mb-6">No credit card needed</div>
              <ul className="space-y-3 text-sm text-gray-600 mb-8">
                {['Up to 10 clients','Unlimited loans per client','Payment history','Your business branding'].map(f => (
                  <li key={f} className="flex items-center gap-2"><span className="text-green-500">✓</span>{f}</li>
                ))}
              </ul>
              <Link href="/login" className="block text-center bg-gray-100 text-gray-700 font-semibold py-3 rounded-lg hover:bg-gray-200 transition-colors">
                Start Free
              </Link>
            </div>
            <div className="bg-primary-600 text-white rounded-xl p-8 elev-2">
              ...
              <ul className="space-y-3 text-sm text-blue-100 mb-8">
                {['Up to 100 clients','Everything in Free','Analytics Dashboard','Top borrowers report','Overdue summary'].map(f => (
                  <li key={f} className="flex items-center gap-2"><span className="text-white">✓</span>{f}</li>
                ))}
              </ul>
              <Link href="/login" className="block text-center bg-white text-primary-600 font-semibold py-3 rounded-lg hover:bg-blue-50 transition-colors">
                Upgrade to Pro
              </Link>
            </div>
          </div>
