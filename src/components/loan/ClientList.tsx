...
export default function ClientList({ tenant, isPaid, selectedClientId, onSelectClient }: Props) {
  ...
  async function handleAdd() {
    ...
    try {
      ...
      setErr(e instanceof Error && e.message === 'FREE_LIMIT'
        ? 'Free plan limit (10 clients) reached. Upgrade to Pro.'
  ...
  return (
    <div className="p-3">
      {/* Search */}
      <div className="bg-white rounded-lg flex items-center px-3 h-10 gap-2 mb-3"
      ...
      {/* Client list */}
      <div className="bg-white rounded-lg overflow-hidden mb-3" style={{boxShadow:'0 1px 3px rgba(0,0,0,.1)'}}>
        {loading ? (
          <div className="flex items-center justify-center gap-2 p-8 text-gray-400 text-sm">
            <div className="w-4 h-4 border-2 border-gray-200 border-t-blue-600 rounded-full" style={{animation:'spin .7s linear infinite'}}/>
            Loading…
          </div>
        ) : filtered.length === 0 ? (
          ...
          filtered.map(c => (
            <button key={c.id} onClick={() => onSelectClient(c.id)}
              className={`w-full flex items-center gap-3 px-3 py-3 border-b border-gray-100 last:border-0 text-left transition-colors ${selectedClientId === c.id ? 'bg-blue-50 border-l-2 border-l-blue-600' : 'hover:bg-gray-50'}`}>
              ...
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#BDBDBD" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
            </button>
          ))
        )}
      </div>
      ...
      {!showAdd ? (
        <button onClick={() => !atLimit && setShowAdd(true)}
          className={`w-full py-2.5 rounded-lg font-medium text-sm flex items-center justify-center gap-2 transition-colors ${atLimit ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>
          <UserPlus size={16}/> Add Client
        </button>
      ) : (
        <div className="bg-white rounded-lg p-4" style={{boxShadow:'0 1px 3px rgba(0,0,0,.1)'}}>
          ...
          <div className="mb-3">
            <label className="block text-xs font-semibold text-blue-600 uppercase tracking-wide mb-1">Phone</label>
            <input value={newPhone} onChange={e => setNewPhone(e.target.value.replace(/\D/g,'').slice(0,10))}
              placeholder="10-digit number" maxLength={10}
              className="w-full border-b border-gray-300 focus:border-blue-600 outline-none py-1.5 text-sm bg-transparent"/>
          </div>
          {err && <div className="flex items-center gap-1 text-red-600 text-xs mb-3"><AlertCircle size={13}/>{err}</div>}
