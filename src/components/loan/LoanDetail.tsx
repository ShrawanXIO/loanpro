...
import { getLoans, getClients, getPayments, addPayment, closeLoan } from '@/lib/firestore'
...
export default function LoanDetail({ tenantId, loanId, clientId, isPaid = false, onBack }: Props) {
  ...
  async function handlePayment() {
    ...
    try {
      ...
      // Check if this payment fully cleared the balance
      const updatedPays = await getPayments(tenantId, loanId)
      const updatedSnap = loanSnapshot(loan!, updatedPays)
      if (updatedSnap.pending <= 0 && !loan!.closed) {
        await closeLoan(loanId) // explicitly mark closed in DB
      }

      setPayAmt('')
      await load()
      
      // Notify sibling components (like the ClientDetail sidebar) to refresh instantly
  ...
  function handlePrint() {
    ...
    printDiv.innerHTML = `
      <style>
        ${printStyles}
        @media print { body > *:not(#__loanpro_print__) { display: none !important; } #__loanpro_print__ { display: block !important; } }
        @media screen { #__loanpro_print__ { display: none; } }
      </style>
      ${printBody}
    `
    ...
    <div className="max-w-lg mx-auto p-4 space-y-3 pb-10">
      {toast && (
        <div className={`fixed top-16 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-4 py-3 rounded-xl shadow-xl text-white text-sm font-medium ${
          ...
          <div className="flex items-center gap-2 flex-1">
            {toast.type === 'success' ? <Check size={16} className="flex-shrink-0"/> : <Share2 size={16} className="flex-shrink-0"/>}
            <span>{toast.text}</span>
          </div>
          {toast.showBack && onBack && (
            <button onClick={() => { setToast(null); onBack() }} className="flex items-center gap-1 bg-white/20 hover:bg-white/30 rounded-lg px-2.5 py-1.5 text-xs font-semibold whitespace-nowrap transition-colors">
      ...
      {snap.isOverdue && (
        <div className="bg-red-50 border-l-4 border-red-600 rounded-r-lg p-4 flex gap-3">
          ...
          <div>
            ...
            <div className="text-red-600 text-sm mt-0.5">Extra interest: {fmtINR(snap.overdueSI)} on outstanding {fmtINR(snap.outstandingPrincipal)}</div>
          </div>
        </div>
      )}
      ...
      {isPaid && (
        <div className="flex gap-2">
          <button onClick={handleShare} className="flex-1 flex items-center justify-center gap-2 bg-green-500 text-white py-2.5 rounded-lg text-sm font-semibold hover:bg-green-600 active:bg-green-700 transition-colors"><Share2 size={16}/> Share via WhatsApp</button>
          <button onClick={handlePrint} className="flex-1 flex items-center justify-center gap-2 bg-gray-700 text-white py-2.5 rounded-lg text-sm font-semibold hover:bg-gray-800 active:bg-gray-900 transition-colors"><Printer size={16}/> Print Summary</button>
        </div>
      )}
      ...
      {!snap.isClosed && (
        <div className="bg-white rounded-lg shadow-sm p-4">
          ...
          <select value={payMode} onChange={e => setPayMode(e.target.value)} className="w-full border-b border-gray-300 focus:border-blue-600 outline-none py-1.5 text-sm text-gray-700 bg-transparent mb-4">
            {['Cash','UPI','Bank Transfer','Cheque','Other'].map(m => <option key={m}>{m}</option>)}
          </select>
          {err && <p className="text-red-600 text-xs mb-3">{err}</p>}
          <button onClick={handlePayment} disabled={saving} className="w-full py-2.5 rounded-lg bg-green-600 text-white font-semibold text-sm hover:bg-green-700 disabled:opacity-50 transition-colors">
